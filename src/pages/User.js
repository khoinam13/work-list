...
function User(){
    ...
    const handleAddImage =  async  (e)=>{
        ...
        const reader = new FileReader()  // FileReader(): Một đối tượng trong JavaScript cho phép đọc dữ liệu từ tệp.
        reader.readAsDataURL(file)  // reader.readAsDataURL(file): Bắt đầu việc đọc dữ liệu từ tệp và chuyển đổi nó thành một URL dữ liệu.
        reader.onloadend = ()=>{
            const base64Img = reader.result  // reader.onloadend: Được gọi khi việc đọc tệp hoàn thành. Trong hàm này, chúng ta lấy dữ liệu base64 của ảnh và lưu vào state selectedImage.
            fetch(`http://localhost:3001/user/${userInfo.id}`,{
            method: "PUT",
            headers: {
                'Content-Type': 'applation/json'
            },
                ...
                <div className="col col-lg-12 mb-4 mb-lg-0">
                    ...
                    <div className="row g-0">
                        ...
                        style={{borderTopLeftRadius: ".5rem; border-bottom-left-radius: .5rem"}}>
                            <div className="p-5">
                                <div style={{width: "100%", height: "140px", borderRadius: "50%", backgroundImage:`${userInfo.image}` ? `url(${userInfo.image})` : `url(${userPNG})`, backgroundSize: "cover", backgroundRepeat : "no-repeat"     }}></div>
                                <label htmlFor="file-input" style={{marginTop: "10px", backgroundColor: "#3d79b1", padding: "5px", borderRadius : "6px"}}>Sửa ảnh</label>
                                <input id="file-input" onChange={handleAddImage} type="file" style={{opacity : "0"}}/>
                            </div>
                            <h5>{userInfo.name}</h5>
